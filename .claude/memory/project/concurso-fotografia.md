...
**Estado (2026-09-22, cierre de sesión):** **App desplegada en producción** (`https://concursofotos.mairenadelalcor.net`, CPD `10.158.0.3`). Auth por contraseña, panel de administración completo, recuperación de contraseña por código OTP de 6 dígitos, y ahora también alta de participante + envío de foto en una sola pantalla sin pasar antes por registro (`/enviar-foto`, ver sección de hoy más abajo). Repo en GitHub (`DiputacionSevilla/concursofotos`), commits `74a256d` y `41a1eac` ya pusheados a `origin/main` **pero el servidor de producción todavía NO tiene este último pull** — sigue pendiente el despliegue manual (`git pull && docker compose build --no-cache web && docker compose up -d web`), el acceso SSH directo sigue bloqueado desde este entorno (ver `reference/ssh-deploy-bloqueado.md`). `npm run typecheck` y `npm run lint` pasan limpios. **Modo pruebas sigue ACTIVADO** en la base de datos compartida — recordar comprobar/desactivarlo antes de que arranque el concurso real.
...
## 2026-09-22 — "Enviar foto sin registro" (alta de participante + foto en 1 sola pantalla)

Motivación: la gente era reticente a registrarse (registro → completar-perfil → participar, 3 pantallas) solo para enviar una foto.

- **Botón verde "Enviar foto sin registro"** en `/login`, lleva a la nueva ruta pública **`/enviar-foto`**.
- `/enviar-foto` es un único formulario que junta: email (bloqueado/prellenado si ya hay sesión con perfil incompleto), todos los campos que antes solo se pedían en `/completar-perfil` (nombre, apellidos, fecha de nacimiento, teléfono opcional, tutor/a si es menor, aceptar bases) y los campos de la foto (título, categoría, descripción, archivo).
- **Server Action `src/features/submissions/services/create-public-submission.ts`**: si no hay sesión, crea la cuenta con `admin.auth.admin.createUser` (contraseña aleatoria `crypto.randomUUID()`, invisible para el usuario), completa el perfil, e inmediatamente hace `signInWithPassword` con el cliente normal (no admin) — a partir de ahí sube la foto e inserta la `submission` con ESE cliente ya autenticado, no con el cliente admin/service-role, para que pase por las RLS normales y por el trigger `enforce_submission_rules()` completo (incluida la ventana de fechas, que el service-role sí se salta — ver esa función en "Triggers/funciones clave" arriba). Si ya hay sesión con perfil incompleto, reutiliza esa cuenta en vez de crear una nueva.
- **Decisión de seguridad**: si el email ya pertenece a una cuenta existente, se rechaza con "Ya existe una cuenta con este email. Inicia sesión para subir tu foto." — NO se adjunta la foto silenciosamente a esa cuenta (a diferencia de `ManualSubmissionForm`/`manual-submission-actions.ts`, la herramienta de alta manual del admin, que sí lo hace porque la opera un empleado de confianza). Evita que cualquiera use el email de otra persona para consumirle su cupo de fotos.
- Se extrajo `mapInsertError` (antes duplicado en `submissions-client.ts` y `manual-submission-actions.ts`) a `src/features/submissions/utils/map-insert-error.ts`.
- Nuevo componente `PublicSubmissionForm.tsx`, adaptado del patrón ya existente en `ManualSubmissionForm.tsx` pero con el checkbox real de aceptación de bases (no la confirmación del admin).
- Verificado end-to-end en navegador contra la BD compartida real (modo_pruebas activado): alta completa, subida, aparición en "Mi foto" como pendiente, y bloqueo correcto del email duplicado. Datos de prueba limpiados manualmente después (usuario `prueba.sinregistro@example.com`).
- Antes de tocar código se hizo un commit de checkpoint del estado previo (`74a256d`). El commit de la feature es `41a1eac`, ya pusheado a `origin/main`.
